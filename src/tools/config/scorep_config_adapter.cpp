...
SCOREP_Config_CompilerAdapter::checkArgument( const std::string& arg )
{
    ...
    std::cerr << "[DEBUG] SCOREP_Config_CompilerAdapter::checkArgument: 357" << std::endl;
...
//#endif /*HAVE_BACKEND( GCC_PLUGIN_SUPPORT )*/
    ...
    if ( arg == "--llvm-filter" )
    {
        m_cflags += "-Xclang -load -Xclang /home/jp/all_repos/gl-sc-llvm-instrumentation/build/lib/instrumentationlib.so ";
        return true;
    }
                                          ...
                                          bool /* nvcc */ )
{
    ...
    if ( m_cflags != "")
    {
      std::cerr << "[DEBUG]: Setting clang arguments for compiler instrumentation to:" << std::endl;
      std::cerr << "[DEBUG] m_cflags = " << m_cflags << std::endl;
      cflags += m_cflags;
    }
    else
    {
