...
#define LLVM_PLUGIN_PATH "/home/jp/all_repos/gl-sc-llvm-instrumentation/build/lib/instrumentationlib.so"
...
SCOREP_Instrumenter_CompilerAdapter::getConfigToolFlag( SCOREP_Instrumenter_CmdLine& cmdLine )
{
    ...
    const std::vector<std::string>& filter_files = cmdLine.getInstrumentFilterFiles();
    assert(filter_files.size() < 2 && "Only single filter files currently supported");
    for ( std::vector<std::string>::const_iterator file_it = filter_files.begin();
          file_it != filter_files.end(); ++file_it)
    {
        //flags += " -Xclang -load -Xclang " LLVM_PLUGIN_PATH " -mllvm --filter-list=" + *file_it;
        flags += " --llvm-filter --filter-list=" + *file_it;
    }
